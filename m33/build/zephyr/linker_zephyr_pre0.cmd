...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x1000000 + 0x0), LENGTH = (0xac800 - 0x0)
    RAM (wx) : ORIGIN = 0x30000000, LENGTH = (256 * 1K)
    NULL_PTR_DETECT ( rw ) : ORIGIN = (0), LENGTH = (1024)
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    NULL_PTR_DETECT (NOLOAD) : { __NULL_PTR_DETECT_start = .; KEEP(*(NULL_PTR_DETECT)) KEEP(*(NULL_PTR_DETECT.*)) __NULL_PTR_DETECT_end = .; } > NULL_PTR_DETECT __NULL_PTR_DETECT_size = __NULL_PTR_DETECT_end - __NULL_PTR_DETECT_start; __NULL_PTR_DETECT_load_start = LOADADDR(NULL_PTR_DETECT);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = 0x30000000 + (256 * 1K);
